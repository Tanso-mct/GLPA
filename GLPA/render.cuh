...
#define JUDGE_V_ON_POLY_FACE(result, result_index, face_dot, line_i, view_volume_vs, vv_line_v_index, poly_vec_1, poly_vec_2, poly_vec_3) \
{ \
    ...
    { \
        ...
        { \
            result[result_index + AX] = view_volume_vs[vv_line_v_index*3 + AX]; \
            result[result_index + AY] = view_volume_vs[vv_line_v_index*3 + AY]; \
            result[result_index + AZ] = view_volume_vs[vv_line_v_index*3 + AZ]; \
        } \
    } \
}

#define GET_POLY_ON_FACE_INXTN(result, line_index, face_dot, view_volume_vs, vv_line_index_1, vv_line_index_2, poly_vec_1, poly_vec_2, poly_vec_3) \
{ \
    ...
    { \
        ...
        { \
            result[line_index + AX] = inxtn[AX]; \
            result[line_index + AY] = inxtn[AY]; \
            result[line_index + AZ] = inxtn[AZ]; \
        } \
    } \
}

#define JUDGE_V_ON_VV_FACE(result, result_index, face_dot, poly_v, face_i, view_volume_vs, vv_face_vs_index) \
{ \
    int vOnFaceIF = (face_dot == 0) ? TRUE : FALSE; \
    for (int conditionalBranch3; conditionalBranch3 < vOnFaceIF; conditionalBranch3++) \
    { \
        ...
        { \
            result[result_index + AX] = poly_v[AX]; \
            result[result_index + AY] = poly_v[AY]; \
            result[result_index + AZ] = poly_v[AZ]; \
        } \
    } \
}

#define GET_POLY_ON_LINE_INXTN(result, result_index, poly_line_v1, poly_line_v2, face_dot, view_volume_vs, vv_face_vs_index, face_index) \
{ \
    int calcInxtnIF  = ((face_dot[0] > 0 && face_dot[1] < 0) || (face_dot[0] < 0 && face_dot[1] > 0)) ? TRUE : FALSE; \
    for(int conditionalBranch3 = 0; conditionalBranch3 < calcInxtnIF; conditionalBranch3++) \
    { \
        ...
        for (int conditionalBranch4 = 0; conditionalBranch3 < inxtnInVvFaceIF; conditionalBranch3++) \
        { \
            result[result_index + AX] = inxtn[AX]; \
            result[result_index + AY] = inxtn[AY]; \
