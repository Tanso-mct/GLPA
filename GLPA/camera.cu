...
void Camera::rasterize(
    ...
    if ((*ptRS)[rsI].scPixelVs.vs[v0I].x == (*ptRS)[rsI].scPixelVs.vs[v1I].x){
    ...
    else if ((*ptRS)[rsI].scPixelVs.vs[v0I].y == (*ptRS)[rsI].scPixelVs.vs[v1I].y){
        inputSideScRvs(leftSideScVs, rightSideScVs, currentSize, (*ptRS)[rsI].scPixelVs.vs[v0I], scYMin);
        inputSideScRvs(leftSideScVs, rightSideScVs, currentSize, (*ptRS)[rsI].scPixelVs.vs[v1I], scYMin);
        ...
        slope
        = ((*ptRS)[rsI].scPixelVs.vs[v1I].y - (*ptRS)[rsI].scPixelVs.vs[v0I].y) /
        ((*ptRS)[rsI].scPixelVs.vs[v1I].x - (*ptRS)[rsI].scPixelVs.vs[v0I].x);

        if (slope >= 0 && (*ptRS)[rsI].scPixelVs.vs[v0I].y > (*ptRS)[rsI].scPixelVs.vs[v1I].y){
            signX = -1;
            signY = -1;
        }
        else if(slope >= 0 && (*ptRS)[rsI].scPixelVs.vs[v0I].y < (*ptRS)[rsI].scPixelVs.vs[v1I].y){
            signX = 1;
            signY = 1;
        }
        else if(slope < 0 && (*ptRS)[rsI].scPixelVs.vs[v0I].y > (*ptRS)[rsI].scPixelVs.vs[v1I].y){
        ...
        while (nowScPixel.y != (*ptRS)[rsI].scPixelVs.vs[v1I].y){
            ...
            = ((*ptRS)[rsI].scPixelVs.vs[v1I].y - (*ptRS)[rsI].scPixelVs.vs[v0I].y) /
            ((*ptRS)[rsI].scPixelVs.vs[v1I].x - (*ptRS)[rsI].scPixelVs.vs[v0I].x) *
            (nextScPixel.x - (*ptRS)[rsI].scPixelVs.vs[v0I].x) + (*ptRS)[rsI].scPixelVs.vs[v0I].y;
    ...
    if (i < polyAmount){
        if (j < sideVsSize[i]){
            for(int k = 0; k <= rightSideScVs[sumSideVsSize[i]*2 + j*2] - leftSideScVs[sumSideVsSize[i]*2 + j*2]; k++){
                rasterizeVs[rsSumSize[i]*3 + sumSidePerSize[sumSideVsSize[i] + i + j]*3 + k*3]
                = ((polyCamOneNs[i*3]*polyCamOneVs[i*3] + polyCamOneNs[i*3 + 1]*polyCamOneVs[i*3 + 1] + polyCamOneNs[i*3 + 2]*polyCamOneVs[i*3 + 2]) /
                (polyCamOneNs[i*3]*(nearScSize[0] * (leftSideScVs[sumSideVsSize[i]*2 + j*2] + k) / scPixelSize[0] - nearScSize[0] / 2) + polyCamOneNs[i*3 + 1]*(nearScSize[1] / 2 - nearScSize[1] * (leftSideScVs[sumSideVsSize[i]*2 + j*2 + 1]) / scPixelSize[1]) + polyCamOneNs[i*3 + 2]*-nearZ)) *
                (nearScSize[0] * (leftSideScVs[sumSideVsSize[i]*2 + j*2] + k) / scPixelSize[0] - nearScSize[0] / 2);

                rasterizeVs[rsSumSize[i]*3 + sumSidePerSize[sumSideVsSize[i] + i + j]*3 + k*3 + 1]
                = ((polyCamOneNs[i*3]*polyCamOneVs[i*3] + polyCamOneNs[i*3 + 1]*polyCamOneVs[i*3 + 1] + polyCamOneNs[i*3 + 2]*polyCamOneVs[i*3 + 2]) /
                (polyCamOneNs[i*3]*(nearScSize[0] * (leftSideScVs[sumSideVsSize[i]*2 + j*2] + k) / scPixelSize[0] - nearScSize[0] / 2) + polyCamOneNs[i*3 + 1]*(nearScSize[1] / 2 - nearScSize[1] * (leftSideScVs[sumSideVsSize[i]*2 + j*2 + 1]) / scPixelSize[1]) + polyCamOneNs[i*3 + 2]*-nearZ)) *
                (nearScSize[1] / 2 - nearScSize[1] * (leftSideScVs[sumSideVsSize[i]*2 + j*2 + 1]) / scPixelSize[1]);

                rasterizeVs[rsSumSize[i]*3 + sumSidePerSize[sumSideVsSize[i] + i + j]*3 + k*3 + 2]
                = ((polyCamOneNs[i*3]*polyCamOneVs[i*3] + polyCamOneNs[i*3 + 1]*polyCamOneVs[i*3 + 1] + polyCamOneNs[i*3 + 2]*polyCamOneVs[i*3 + 2]) /
                (polyCamOneNs[i*3]*(nearScSize[0] * (leftSideScVs[sumSideVsSize[i]*2 + j*2] + k) / scPixelSize[0] - nearScSize[0] / 2) + polyCamOneNs[i*3 + 1]*(nearScSize[1] / 2 - nearScSize[1] * (leftSideScVs[sumSideVsSize[i]*2 + j*2 + 1]) / scPixelSize[1]) + polyCamOneNs[i*3 + 2]*-nearZ)) *
                -nearZ;

                rasterizePixelVs[rsSumSize[i]*2 + sumSidePerSize[sumSideVsSize[i] + i + j]*2 + k*2]
                = leftSideScVs[sumSideVsSize[i]*2 + j*2] + k;

                rasterizePixelVs[rsSumSize[i]*2 + sumSidePerSize[sumSideVsSize[i] + i + j]*2 + k*2 + 1]
...
void Camera::zBuffer(std::vector<RasterizeSource>* ptRS, double* &zbRSIs, double* &zbVs, double* &zbComp){
    int scYMax = 0;
    ...
    for (int i = 0; i < ptRS->size(); i++){
        if ((*ptRS)[i].scPixelVs.vs.size() >= 3){
            ...
            sideScVsSize += scYMax - scYMin + 1;
            rasterizeTargetAmount += 1;
    ...
    double* hRightSideScVs = new double[sideScVsSize*2]{-2.0}; 
    std::fill(hRightSideScVs, hRightSideScVs + sideScVsSize*2, -2.0); 

    double* hPolyCamOneVs = (double*)malloc(sizeof(double)*rasterizeTargetAmount*3);
    ...
    for (int i = 0; i < ptRS->size(); i++){
        if ((*ptRS)[i].scPixelVs.vs.size() >= 3){
            ...
            hPolyCamOneVs[polyAmount*3 + 0] = (*ptRS)[i].polyCamVs[0].x;
            hPolyCamOneVs[polyAmount*3 + 1] = (*ptRS)[i].polyCamVs[0].y;
            hPolyCamOneVs[polyAmount*3 + 2] = (*ptRS)[i].polyCamVs[0].z;

            hPolyCamOneNs[polyAmount*3 + 0] = (*ptRS)[i].polyN.x;
            hPolyCamOneNs[polyAmount*3 + 1] = (*ptRS)[i].polyN.y;
            hPolyCamOneNs[polyAmount*3 + 2] = (*ptRS)[i].polyN.z;

            hSideVsSize[polyAmount] = scYSize;
            hSumSideVsSize[polyAmount] = currentSize;
            rsI.push_back(i);
    ...
    int dataSizeY = polyAmount;
    int dataSizeX = maxRasterizeSize;
    int desiredThreadsPerBlockX = 16;
    int desiredThreadsPerBlockY = 16;
    ...
    zbVs = new double[(scPixelSize.x + 1) * (scPixelSize.y + 1) * 3];

    zbComp = new double[(scPixelSize.x + 1) * (scPixelSize.y + 1)];
    ...
    for (int i = 0; i < rsI.size(); i++){
        for(int j = 0; j < rsSumSize[i+1] - rsSumSize[i]; j++){
            if (
                zbComp[static_cast<int>(hRasterizePixelVs[workI*2] + (hRasterizePixelVs[workI*2 + 1])*(scPixelSize.x + 1))] >=
                ...
                zbComp[static_cast<int>(hRasterizePixelVs[workI*2] + hRasterizePixelVs[workI*2 + 1]*(scPixelSize.x + 1))] = hRasterizeVs[workI*3 + 2] / -farZ;
            }
